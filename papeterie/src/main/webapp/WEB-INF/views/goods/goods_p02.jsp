   	...
   	<ul class="paging">
   		<!-- 10페이지 이전 -->
   	 	<c:if test="${pstart != 1}">
   	 		<li>
   	 			<div class="pagingBox">
   	 				<a href="../goods/goods_p02?page=${pstart-10}">◀맨앞</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${pstart == 1}">
		...
		<c:if test="${page != 1 }">
   	 		<li>
   	 			<div class="pagingBox" style="margin-right:10px;">
   	 				<a href="../goods/goods_p02?page=${page-1}">◁이전</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${page == 1}">
   		...
   		<c:forEach begin="${pstart}" end="${pend}" var="i">
	   	 	...
	   	 	<c:if test="${page == i}">
	   	 		...
	   	 		<li><a id="pagingPage" href="../goods/goods_p02?page=${i}" ${str}>${i}</a></li>
	   	 	</c:forEach>
   		...
   		<!-- 다음페이지 -->	
   	 	<c:if test="${page != chong}">
  	 		<li>
  	 			<div class="pagingBox">
  	 				<a href="../goods/goods_p02?page=${page+1}">다음▷</a>
  	 			</div>
  	 		</li>
	 	</c:if>
  	 	<c:if test="${page == chong}">
  	 	...
  	 	<c:if test="${pend != chong}">
   	 		<li>
   	 			<div class="pagingBox">
   	 				<a href="../goods/goods_p02?page=${chong}">맨뒤▶</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${pend == chong}">
